...
- If NumPy fails to import with errors like `ImportError: No such process` under Cygwin,
  reinstall NumPy (`pip install --force-reinstall numpy`) or run with a native Python
  distribution (standard Windows Python or Linux Python).

### Run tests

```bash
python -m unittest tests/test_evm.py
```
